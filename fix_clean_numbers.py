...
spotify_dl_body = '''def main():
    while True:
        clear_screen()
        print(f"  {C_CYAN}[1]{C_RESET} Spotify Song / Album / Playlist Downloader (Quality & Size Info)")
        ...
        if choice == '1':
            while True:
                ...
                elif qc in ['*', 'x', 'exit', 'q']: os._exit(0)

                bitrate = "160k" if qc == '2' else ("96k" if qc == '3' else "320k")
                print(f"\\n{C_YELLOW}[*] Downloading tracks ({bitrate}) into {out_dir}...{C_RESET}")
                subprocess.run(["spotdl", url, "--output", out_dir, "--audio", bitrate])
            ...
            while True:
                clear_screen()
                print(f"{C_YELLOW}[*] Spotify Video / Canvas Downloader{C_RESET}")
                print(f"{C_CYAN}─"*60 + f"{C_RESET}")
...
# 4. Converter (1, 2, 3, 4, 5)
converter_body = '''def main():
    while True:
        clear_screen()
        print(f"  {C_CYAN}[1]{C_RESET} Extract Audio: Video to MP3 (320kbps High Quality)")
        print(f"  {C_CYAN}[2]{C_RESET} Convert Video Format (MKV/AVI/MOV/WEBM to MP4)")
        print(f"  {C_CYAN}[3]{C_RESET} Convert Audio Format (WAV/AAC/OGG/M4A to MP3)")
        print(f"  {C_CYAN}[4]{C_RESET} Compress Video (Reduce File Size)")
        print(f"  {C_CYAN}[5]{C_RESET} Convert Video to Animated GIF")
        print(f"{C_CYAN}─"*60 + f"{C_RESET}")
        print(f"  {C_CYAN}[#]{C_RESET} Back to Main Menu Hub")
        print(f"  {C_RED}[*]{C_RESET} Full Exit to Terminal")
        print(f"{C_CYAN}═"*60 + f"{C_RESET}")

        choice = input(f"{C_GREEN}➤ Select Option (1-5): {C_RESET}").strip().lower()
        if choice in ['#', 'b', 'back']: break
        elif choice in ['*', 'x', 'exit', 'q']: os._exit(0)
        ...
        if choice in ['1', '2', '3', '4', '5']:
            while True:
                clear_screen()
                titles = {
                    '1': "Video to MP3 Extraction",
                    '2': "Video to MP4 Conversion",
                    '3': "Audio to MP3 Conversion",
                    '4': "Video Compression",
                    '5': "Video to GIF"
                }
                print(f"{C_YELLOW}[*] Mode: {titles[choice]}{C_RESET}")
                print(f"{C_CYAN}─"*60 + f"{C_RESET}")
                v_path = input(f"{C_GREEN}➤ Enter Full File Path ([#] Back / [*] Exit): {C_RESET}").strip().strip("'\\"")
                if v_path.lower() in ['#', 'b', 'back']: break
                elif v_path.lower() in ['*', 'x', 'exit', 'q']: os._exit(0)

                if not os.path.exists(v_path):
                    input(f"\\n{C_RED}❌ Path not found! Press Enter...{C_RESET}")
                    continue

                base = v_path.rsplit(".", 1)[0]
                if choice == '1': cmd = ["ffmpeg", "-i", v_path, "-vn", "-ab", "320k", base + ".mp3", "-y"]
                elif choice == '2': cmd = ["ffmpeg", "-i", v_path, "-c:v", "libx264", "-c:a", "aac", base + "_converted.mp4", "-y"]
                elif choice == '3': cmd = ["ffmpeg", "-i", v_path, "-ab", "320k", base + "_converted.mp3", "-y"]
                elif choice == '4': cmd = ["ffmpeg", "-i", v_path, "-vcodec", "libx264", "-crf", "28", base + "_compressed.mp4", "-y"]
                elif choice == '5': cmd = ["ffmpeg", "-i", v_path, "-vf", "fps=10,scale=320:-1:flags=lanczos", base + ".gif", "-y"]

                print(f"\\n{C_YELLOW}[*] Processing with FFmpeg... Please wait...{C_RESET}")
                subprocess.run(cmd)
                input(f"\\n{C_GREEN}✅ Conversion Completed! Press Enter...{C_RESET}")
                break

if __name__ == "__main__": main()'''

# 5. Telegram Tools (1, 2, 3)
...
files = {
    ...
    "converter.py": wrap_script("ADVANCED MEDIA CONVERTER", converter_body),
...
print("✅ All menus cleaned up! Leading zeros removed: Clean 1, 2, 3... 10 format applied everywhere.")